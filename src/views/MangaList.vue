...
<script>
  ...
  export default {
    ...
    methods: {
      loadNewEntrys() {
        if (this.hasNextPage){
          DatabaseHandler.queryMediaPaged(this.currentPage + 1, DatabaseHandler.TYPE.MANGA).then((data) => {
  ...
  created() {
      ...
      DatabaseHandler.queryMediaPaged(this.currentPage, DatabaseHandler.TYPE.MANGA).then((data) => {
        this.currentPage = data.Page.pageInfo.currentPage;
        this.hasNextPage = data.Page.pageInfo.hasNextPage;
